...
- Added new MFA-related audit log actions: `ENABLE_MFA_ENFORCEMENT`,
...
- Removed "Preview" label from the `S3VpcEndpointId` field description in the
  `Region` model, indicating the field has graduated to general availability.
- Updated `ListInvoices` `start_time` and `end_time` parameter descriptions to
  specify that values must be in RFC3339 format with examples.
