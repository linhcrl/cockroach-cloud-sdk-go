...
echo "Invoking Claude to analyze changes and update changelog..." >&2

# Get the script directory to find the prompt template
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROMPT_TEMPLATE="$SCRIPT_DIR/../prompts/update-changelog.md"
...
echo "Claude response received, extracting metadata..." >&2
...
if [[ -z "${COMMIT_TITLE:-}" ]]; then
  echo "::error::Could not extract commit title from Claude output" >&2
...
if [[ -z "${PR_TITLE:-}" ]]; then
  echo "::error::Could not extract PR title from Claude output" >&2
  echo "Claude output:" >&2
  echo "$RESULT_TEXT" >&2
  rm --force "$PROMPT_FILE" "$OUTPUT_FILE"
  exit 1
